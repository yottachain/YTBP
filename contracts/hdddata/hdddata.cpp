...
const symbol hdd_symbol = symbol("HDD", 4);
const name hdd_account = "hddofficial"_n;
const uint32_t reg_hdd_per_sqm = 1;
...
void hdddata::registerland(name owner, double lat_north_edge,
    double long_east_edge, double lat_south_edge, double long_west_edge)
{
    ...
    asset hdd_amount = asset(0, hdd_symbol);
    ...
    while(lands_itr != lat_north_index.end() && lands_itr->lat_north_edge < upper_bound)
    {
            ...
            l.long_west_edge >= long_west_edge)
        {
            ...
            hdd_amount -= calculate_land_reg_fee(covered_land_area, reg_hdd_per_sqm, reg_years_left);
    ...
    hdd_amount += calculate_land_reg_fee(land_size, reg_hdd_per_sqm, 1);
    if(hdd_amount.amount < 1 * 10000)
    {
        hdd_amount = asset(1 * 10000, hdd_symbol);
    }
    deduct_hdd_deposit(owner, hdd_amount);

    // The registration fee gets sent back to the token issuing account
    transfer_hdd(_self, hdd_account, hdd_amount, "Land registration fee");
...
void hdddata::moveland(uint64_t land_id, double lat_north_edge,
    double long_east_edge, double lat_south_edge, double long_west_edge)
{
    ...
    asset old_land_fee = calculate_land_reg_fee(old_land_size, reg_hdd_per_sqm, 1);

    std::pair<double, double> new_land_size = assert_lat_long_values(lat_north_edge, long_east_edge,
        lat_south_edge, long_west_edge);
    asset new_land_fee = calculate_land_reg_fee(new_land_size, reg_hdd_per_sqm, 1);
...
void hdddata::setlandprice(uint64_t land_id, asset price)
{
    name owner = require_land_owner_auth(land_id);

    eosio_assert(price.symbol == hdd_symbol, "You must set a price in hdd");
...
void hdddata::cancelsale(uint64_t land_id)
{
    name owner = require_land_owner_auth(land_id);
...
void hdddata::buyland(name buyer, uint64_t land_id, asset price)
{
    require_auth(buyer);
    ...
    deduct_hdd_deposit(buyer, lp.price);
    transfer_hdd(_self, owner, lp.price, "Your land has been purchased!");
...
void hdddata::persistpoly(uint64_t land_id, std::string poly_id,
...
void hdddata::updatepersis(uint64_t persistent_id, uint64_t land_id,
    vector3 position, vector3 orientation, vector3 scale)
{
    persistent_table persistents(_self, _self.value);
...
void hdddata::deletepersis(uint64_t persistent_id)
{
    persistent_table persistents(_self, _self.value);
...
void hdddata::opendeposit(name owner)
{
    require_auth(owner);
    ...
    if(deposits_itr == deposits.end())
    {
        deposits.emplace(owner, [&](auto &row) {
            row.owner = owner;
            row.balance = asset(0, hdd_symbol);
        });
    }
}

void hdddata::closedeposit(name owner)
{
    ...
    {
        transfer_hdd(_self, owner, d.balance, "hdd deposit refund");
    }

    deposits.erase(deposits.find(owner.value));
}

void hdddata::deposithdd(name from, name to, asset quantity, std::string memo)
{
    // In case the tokens are from us, or not to us, do nothing
    if(from == _self || from == hdd_account || to != _self)
        return;
    // This should never happen as we ensured transfer action belongs to "hddofficial" account
    eosio_assert(quantity.symbol == hdd_symbol, "The symbol does not match");
...
std::pair<double, double> hdddata::assert_lat_long_values(const double& lat_north_edge,
...
void hdddata::check_land_intersections(const land_table& lands, const double& lat_north_edge,
...
asset hdddata::calculate_land_reg_fee(const std::pair<double, double>& land_size, const uint32_t& hdd_per_sqm, const double& years)
{
    ...
    return calculate_land_reg_fee(land_area, hdd_per_sqm, years);
}

asset hdddata::calculate_land_reg_fee(const double& land_area, const uint32_t& hdd_per_sqm, const double& years)
{
    int64_t reg_price = static_cast<int64_t>(round(land_area * hdd_per_sqm * years));
    // multiply fee by 10000 to account for four decimal places of hdd
    return asset(reg_price * 10000, hdd_symbol);
}

void hdddata::deduct_hdd_deposit(const name& owner, const asset& hdd_amount)
{
    deposit_table deposits(_self, _self.value);
    deposit d = deposits.get(owner.value, "User does not have a deposit opened");
    eosio_assert(d.balance >= hdd_amount, "User's hdd deposit balance is too low");

    deposits.modify(deposits.find(owner.value), same_payer, [&](auto &row){
        row.balance -= hdd_amount;
    });
}

uint64_t hdddata::get_land_id_from_persistent(const persistent_table& persistents, const uint64_t& persistent_id)
{
    persistent p = persistents.get(persistent_id, "Persistent Id does not exist");
    return p.land_id;
}

name hdddata::require_land_owner_auth(const uint64_t& land_id)
{
    land_table lands(_self, _self.value);
    land l = lands.get(land_id, "Land Id does not exist");
...
void hdddata::assert_vectors_within_bounds(const vector3& position,
...
void hdddata::delete_persistents_from_land(const uint64_t& land_id)
{
    persistent_table persistents(_self, _self.value);
...
// cleos set account permission hdddata1 active '{"threshold": 1,"keys": [{"key": "ACTIVE PUBKEY","weight": 1}],"accounts": [{"permission":{"actor":"hdddata1","permission":"eosio.code"},"weight":1}]}' owner -p hdddata1@owner
void hdddata::transfer_hdd(name from, name to, asset quantity, std::string memo)
{
    action{
...
extern "C" {
    void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        if(code==receiver)
        {
            switch(action)
            {
                EOSIO_DISPATCH_HELPER( hdddata, (registerland)(moveland)(persistpoly)(updatepersis)(deletepersis)(opendeposit)(closedeposit)(setlandprice)(cancelsale)(buyland) )
            }
        }
